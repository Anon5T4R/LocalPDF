...
//! Mesmo padrão dos irmãos da suíte; o LocalPDF prefere a porta 8102
//! (Writer=8088, Code=8090+, Sheets=8099+, Slides=8100+, Data=8101+, Chat=8103+).
...
/// LocalPDF prefere a porta 8102. Se estiver ocupada (outro app da suíte, um
/// servidor órfão ou uma segunda janela), cai pra próxima porta livre da faixa.
fn pick_free_port() -> Result<u16, String> {
    for port in 8102u16..=8122 {
        if TcpListener::bind(("127.0.0.1", port)).is_ok() {
            return Ok(port);
        }
    }
    Err("nenhuma porta livre entre 8102 e 8122 para a IA".into())
}
